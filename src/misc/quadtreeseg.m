function [point idp rect] = quadtreeseg( im, varargin )

% This function is modified from the following Author:
...
%   @date: June 2010
    ...
    % modifications for ATSUI
    zs   = reshape(im,sz(1)*sz(2),1);
    mesh = reshape(seg_map,sz(1)*sz(2),1);
    mesh = mesh+1; % avoid zero index

    [point idp] = datapoint(zs,mesh);
    % Rectangle for plotting
    rect = seg_list(1:size(point,1),:); 
...
function [maxA id] = datapoint(A,B)

Abs  = abs(A);
% maxi = accumarray(B,A,[],@max);
id   = accumarray(B,(1:numel(Abs)).',[], @(x) maxmod(Abs,x));
maxA = A(id);
end
